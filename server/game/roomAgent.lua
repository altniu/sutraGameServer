...
local function newRoomID()
  ...
  datacenter.set("serial_id", "room_id", serialid+1)
  
	return sercial_room_id
end
...
local function gameOverHandle()
	...
	local function handleFunc()
		skynet.sleep(100)
		for k,v in pairs(m_room.playerList) do
			skynet.call(v.player.agent, "lua", "gameOverNotify", m_room:getCurrentPaySite(), "0")
		end
	end
...
local function pokesLoop()
	...
	local function pokes()
		skynet.sleep(100)
		--广播
		local dizhuSite = m_room:getDizhuBySite()
		for k,v in pairs(m_room.playerList) do
		...
		do
			...
			if sleepTime >= m_chupai_time then
				local paysite = m_room:getCurrentPaySite()
				local code, roundWinSite, isWin = m_room:payPoke(paysite, {})
			...
			if m_chupai_site ~= 0 then
				...
				--广播
				local lastPokes = m_room:getLastPokes()
				local pokes = ""
			...
			if m_playerExit_site ~= 0 then
				...
				for k,v in pairs(m_room.playerList) do
					if v.player then
						--local iswin = m_room:playerIsWinWithSite(v.player.site)
	...
	local function qiangdizhu()
		...
		do
			...
			if m_qiangdizhuIndex ~= 0 then
				...
				m_room:qiangdizhuFunc(m_qiangdizhuIndex > 0)

				--广播
				for k,v in pairs(m_room.playerList) do
					skynet.call(v.player.agent, "lua", "qiangdizhuNotify", m_room:getQiangDizhuSite(), m_qiangdizhuIndex > 0)
				end
				m_qiangdizhuIndex = 0
			end
	...
	local function jiaodizhu()
		...
		do
			...
			if m_jiaodizhuIndex ~= 0 then
				...
				m_room:jiaodizhuFunc(m_jiaodizhuIndex > 0)

				--广播 叫地主
				for k,v in pairs(m_room.playerList) do
					if v.player then 
						skynet.call(v.player.agent, "lua", "jiaodizhuNotify", m_room:getJiaoDizhuSite(), m_jiaodizhuIndex > 0)
					end
				end
				m_jiaodizhuIndex = 0
			end
...
local function startGame()
	local function startPoke()
		...
		for k,v in pairs(m_room.playerList) do
			...
			local secretPokes = ""
			local roomSecretPoke = m_room:getSecretPoke()
			...
			skynet.call(v.player.agent, "lua", "startGameNotify", pokes, secretPokes, m_room:getJiaoDizhuSite())
		end
...
function CMD.playerEnter(playerTmp)
	...
	if m_room.playerCount >= DDZ_Define.play_MaxCount then
		print("roomAgent : people is full")
		return 0
	end
	local playerUID = player.uid
	player = m_room:addPlayer(player)
	local players = {}
	for k, v in pairs(m_room.playerList) do
		if v.player then
			local pInfo = {}
	...
	return m_room.id, player.site, players
end


function CMD.playerReady(roomID, uid, isReady)
	if m_room.id == roomID then
		m_room:playerReady(uid, isReady)
		for k,v in pairs(m_room.playerList) do
...
function CMD.playerPayPoke(site, pokes)
	...
	local isvalid = m_room:checkPayPoke(site, pokesValue)
	if not isvalid then
		return 1
	end

	local code, roundWinSite, isWin, pokeType = m_room:payPoke(site, pokesValue)
	m_chupai_site = site
	m_chupai_roundWinSite = roundWinSite
...
function CMD.playerExitRoom(site)
	...
	local isPlaying = m_room:isPlaying()
	m_room:playerExit(site)
